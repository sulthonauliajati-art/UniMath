...
async function run() {
  ...
  const newCsvPath = path.resolve(process.cwd(), '../../checkpoint uji pemahaman.csv')
  if (!fs.existsSync(newCsvPath)) {
    console.error(`❌ File "checkpoint uji pemahaman.csv" tidak ditemukan di: ${newCsvPath}`)
    process.exit(1)
  }
  ...
  // Read Master CSV
  const masterCsvPath = path.resolve(process.cwd(), '../../unimath_master_list_materi_M1A-M1F_R1-R3.csv')
  if (!fs.existsSync(masterCsvPath)) {
    console.error('❌ File master tidak ditemukan!')
    process.exit(1)
  }
  ...
  if (process.env.TURSO_DATABASE_URL && process.env.TURSO_AUTH_TOKEN) {
    ...
    console.log('⚠️ TURSO_DATABASE_URL atau TURSO_AUTH_TOKEN tidak ditemukan di .env.local. Sinkronisasi database dilewati.')
  }
}
